...
% Define a range of values for the coefficient associated with x3
x3_coeff_values = [90600, 126600, 162600]; % corresponding to Low, Mean, High

% Initialize arrays to store results
sensitivity_results = zeros(length(x3_coeff_values), 1);

% Perform sensitivity analysis
for i = 1:length(x3_coeff_values)
    % Update the objective function with the new coefficient for x3
    obj = 35000*x1 + 35000*x2 + x3_coeff_values(i)*x3 + 210000*x4 + 108000*x5 + 120000*x6 + 90000*x7 + 300000*x8 + 9000000*x9 + 150000*x10 + 300000*x11;
...
for i = 1:length(x3_coeff_values)
    fprintf('Coefficient for x3: %d, Objective Function Value: %.2f\n', x3_coeff_values(i), sensitivity_results(i));
end
